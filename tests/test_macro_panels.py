...
def test_a_shown_panels_first_canvas_has_real_width(browser, site):
    ...
    canvas measured while its panel is `hidden` is 0px wide."""
    context = browser.new_context(viewport=DESKTOP_VIEWPORT)
    page = context.new_page()
    try:
        page.goto(f"{site}/macro.html", wait_until="load")
        ...
        width = page.evaluate("document.getElementById('historyCanvas').width")
        assert width > 0, "the GDP history canvas is still 0px wide after its panel was shown"
    finally:
        context.close()
