...
def test_indicator_definitions_are_read_from_config_yaml(tmp_path):
    """_indicator_definitions() reads the PUBLIC `definition` block straight
    from config/indicators/*.yaml (and its derived/ subdirectory), because
    that field is never written to the database the way `names` is -- it is
    authoring-time metadata, not sync output."""
    indicators_dir = tmp_path / "indicators"
    indicators_dir.mkdir()
    (indicators_dir / "BIRTHS.yaml").write_text(
        "id: BIRTHS\n"
        "definition:\n"
        "  en: Number of live births registered among residents in a year.\n"
        "  fr: Nombre de naissances vivantes enregistrées parmi les "
        "habitants de la commune au cours de l'année civile.\n"
        "  nl: Aantal levendgeborenen geregistreerd bij inwoners tijdens "
        "het kalenderjaar.\n",
        encoding="utf-8",
    )
    # No `definition` block: must simply be absent from the result, not a
    # KeyError and not a fabricated fallback.
    (indicators_dir / "NO_DEFINITION.yaml").write_text(
        "id: NO_DEFINITION\n",
        encoding="utf-8",
    )
    derived_dir = indicators_dir / "derived"
    derived_dir.mkdir()
    (derived_dir / "AVG_INC.yaml").write_text(
        "id: AVG_INC\n"
        "definition:\n"
        "  en: Average taxable income per tax return in the commune.\n"
        "  fr: Revenu net imposable moyen par déclaration dans la commune.\n"
        "  nl: Gemiddeld netto belastbaar inkomen per aangifte in de gemeente.\n",
        encoding="utf-8",
    )

    definitions = _indicator_definitions(indicators_dir)
    assert set(definitions) == {"BIRTHS", "AVG_INC"}
    assert definitions["BIRTHS"]["en"].startswith("Number of live births")
    assert definitions["AVG_INC"]["fr"].startswith("Revenu net imposable moyen")


def test_indicator_index_carries_definition_through_to_the_published_index(tmp_path):
    """The exporter's public output must actually thread `definition` through
    to metadata/indicators.json -- reading it in `_indicator_definitions`
    alone would be dead code if `_indicator_index` never received it.

    Calls `_indicator_index` directly with a fixture `definitions` map rather
    than routing through `export_site_payloads` (which reads the real
    config/indicators directory and would make this test depend on which
    real indicators happen to have a `definition` block yet)."""
    from export_site_payloads import _indicator_index

    indicators = {
        "POP": {"name": "Population", "unit": "count", "communes": {"11001": {"value": 1}}},
        "BIRTHS": {"name": "Births", "unit": "count", "communes": {"11001": {"value": 2}}},
    }
    names = {"POP": {"en": "Population", "fr": "Population", "nl": "Bevolking"}}
    definitions = {
        "POP": {
            "en": "Number of residents in the commune.",
            "fr": "Nombre d'habitants de la commune.",
            "nl": "Aantal inwoners van de gemeente.",
        }
    }
    ...
    index = _indicator_index(indicators, names, set(), db_path, {}, definitions)
    by_code = {row["indicator_code"]: row for row in index}

    assert by_code["POP"]["definition"] == definitions["POP"]
