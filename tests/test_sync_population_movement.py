...
def test_observation_period_is_the_sheet_year_not_the_following_year(db):
    """The workbook's grid lookup uses sheet_year + 1 (see module docstring),
    but the observation itself belongs to the calendar year the movement
    happened in -- the sheet's own year."""
...
# --- geography: period-correct, the opposite of Wave 1 -----------------------
...
def test_a_2011_row_under_the_old_code_resolves_to_the_geography_valid_then():
    ...
    be:mun:55010 -- checked against the commune grid as of 2012-01-01 (sheet
    year + 1), where 55010 is still the live code -- not to 51067, which did
    not exist yet."""
...
def test_bastogne_2024_row_resolves_only_from_its_valid_from_date(db):
    """82039 (Bastogne+Bertogne) is valid_from 2024-12-02 -- inside sheet
    '2024' itself (as_of = 2025-01-01). A '2024' sheet row under 82039 must
    resolve, matching the real file (verified 2026-09-23: 82039 appears only
    in sheets 2024 and 2025)."""
    raw = _make_workbook({"2024": [_data_row("82039", 50, 40, 3, 1)]})
    sync_population_movement.sync(db, xlsx_bytes=raw)
    rows = _observations(db, "BIRTHS", "be:mun:82039")
    assert rows == [("be:mun:82039", "2024", 50.0, "final")]
