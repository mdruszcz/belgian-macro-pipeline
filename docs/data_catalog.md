...
| Field | Value |
|---|---|
| Publisher | Eurostat — regional economic accounts |
| What | GDP per capita in purchasing power standards, EU27 (from 2020) basis, per inhabitant. Filter used: `unit=PPS_EU27_2020_HAB` — **not** `PPS_HAB_EU27_2020`, which is "in % of the EU27 average", a different series, confirmed from the response's own dimension labels |
| Periods at source | Annual, 2000–2024 (25 years) |
| Coverage | 285 real NUTS 2 regions across the response (0 pseudo-region codes); 276 with a value in the latest year (2024). After the pilot's own country-prefix licence filter: 285 kept, 0 dropped, 0 unresolved. No Swiss, Icelandic or Liechtenstein regions at all — Eurostat does not publish this dataset for EFTA; genuinely partial, not a filtering artefact |
| Geography | NUTS 2, verified against Nuts2json's 2024 vintage geometry (recommended vintage — see spec) |
| Acquisition | `EurostatSource` (`src/fetchers/eurostat.py`), same adapter and transport as the approved `eurostat` row below — **parses cleanly today**, no adapter change needed for this one dataset |
| Source page | `ec.europa.eu/eurostat/databrowser/product/view/nama_10r_2gdp` |
...
### Population — `demo_r_pjanaggr3` (APPROVED by the maintainer 2026-09-14; loaded)

| Field | Value |
|---|---|
| Publisher | Eurostat — regional demographic statistics (a NUTS 3 dataset that also carries NUTS 0–2 rows in the same response) |
| What | Population on 1 January, total, both sexes, all ages. Filters: `sex=T&age=TOTAL&unit=NR` |
| Periods at source | Annual, 1990–2025 (36 years) |
| Coverage | 361 NUTS-2-shaped codes, of which 4 are pseudo-regions (`ALXX`, `FRXX`, `HUXX`, `MKXX` — Eurostat's "Not regionalised/Unknown NUTS 2" label, a different convention from national accounts' `ZZ` extra-regio) → 357 real regions; 296 with a value in the latest year (2025). After licence filter: 314 kept, 41 dropped (40 UK + `EU28`), 1 **unresolved** — `EFTA`, the same aggregate-code hazard as `EA21` above. **Loaded**: 314 regions, 8,326 rows, 1990–2025; 295 with a value in 2025 |
| Geography | NUTS 2, 2024 vintage recommended |
| Acquisition | **Loaded (2026-09-14).** The compound-flag adapter fix (#168) resolved this response's `be`/`bep`/`ep` cells. Its one remaining flag-with-no-value cell (geo=PL912, period=2010, flag `b`) is a 5-character NUTS 3 code this loader never wanted — `EurostatSource._parse` gained an optional `geo_filter` parameter (src/fetchers/eurostat.py) that skips a rejected geography entirely, before its flag/value are ever validated; `scripts/sync_nuts2.py` passes `geo_filter=is_nuts2_code`, which filters PL912 out without touching the `u`/flag-status mapping. See docs/features/europe_nuts2.md, "Coverage", and ADR 0009's 2026-09-14 amendment, item 8 |
...
| Field | Value |
|---|---|
| Publisher | Eurostat/GISCO, distributed via the `eurostat/Nuts2json` GitHub project |
| What | TopoJSON NUTS boundaries, per level (0–3), per NUTS version year, per projection/resolution. Proposed: level 2, projection 3035, resolution 20M, **2024 vintage** — 292 regions, 574,331 bytes measured |
| Periods at source | Multiple NUTS version years published (2010, 2013, 2016, 2021, 2024 confirmed present); this proposal uses 2024 only |
| Coverage | 292 of the ~300+ real NUTS 2 regions across all candidate datasets; matches each dataset's latest year with 10–23 code mismatches (vs. 46–65 under the 2021 vintage) — see the spec's full match table. Some overseas/candidate-country codes (`FRY1`–`FRY5`, `PT20`, `PT30`) present in Eurostat data have no match under either vintage, unresolved, not guessed |
| Geography | NUTS 2, self-hosted under `public/data/geo/nuts2/` if approved — never fetched from GitHub/ec.europa.eu at runtime |
| Acquisition | Static file download, `pub/v2/{year}/3035/20M/2.json`, no adapter, no fetch_runs row |
| Source page | `github.com/eurostat/Nuts2json` |
...
| Field | Value |
|---|---|
| Publisher | Eurostat/GISCO (`eurostat/eurostat-map.js` on GitHub, published to npm as `eurostat-map`) |
| What | A D3-based choropleth/proportional-symbol map renderer built specifically for Eurostat statistics and Nuts2json geometry |
| Version | 4.11.3, published 2026-08-18 (npm `dist-tags.latest`, confirmed 2026-09-14) |
| Format | One self-contained webpack UMD bundle, `build/eurostatmap.min.js`, downloaded and measured: **1,219,896 bytes (1.16 MiB)** minified. Bundles its own `d3-*`, `proj4`, `topojson-client`, `idb-keyval`, `jsonstat-toolkit`, `simple-statistics` — no companion `<script>` tags needed |
| Coverage | Not applicable (a library, not a dataset) |
| Geography | Not applicable — consumes whatever geometry it is pointed at |
| Acquisition | Proposed: vendored under `assets/vendor/eurostat-map/`, never loaded from a CDN (rule 30: the public site stays static and self-contained) |
| Source page | `github.com/eurostat/eurostat-map.js`, `npmjs.com/package/eurostat-map` |
...
| `dbnomics_eurostat` | Eurostat (via DBnomics) — **retired 2026-09-13, replaced by `eurostat`** (row below; international pilot PR 1) | `dbnomics` | `api.db.nomics.world/v22/series/Eurostat` | **Eurostat copyright/licence policy** (text supplied by the maintainer, 2026-09-13) — reuse, including commercial dissemination, authorised with Eurostat acknowledged as the source; **not** for commercial redissemination: data not relating to EU, EFTA or official EU acceding/candidate countries, and data originating from sources other than Eurostat; adapted or modified data must say so prominently. **Exception:** the EU business and consumer surveys (DG ECFIN, disseminated by Eurostat, e.g. `ei_bssi_m_r2` behind `EC_CONS_CONF_BE`/`_EU`) are **CC0**, confirmed by the maintainer 2026-09-13. **Plus DBnomics' own licence on the transport:** DBnomics distributes its aggregated datasets under the **Open Database License (ODbL)** (supplied by the maintainer, 2026-09-13) — attribution to DBnomics, and share-alike: a database derived from it that is publicly used or distributed (committed CSVs in this public repository, paid exports) must itself be offered under ODbL. **Decided 2026-09-13: DBnomics is dropped as a transport; this data moves to `eurostat` (row below).** Conditions as they apply here are in `docs/features/international.md` | daily |
| `eurostat` | Eurostat | `eurostat` (dataset-level, **built** — `src/fetchers/eurostat.py`, international pilot PR 1) | `ec.europa.eu/eurostat/api/dissemination` | **Eurostat copyright/licence policy**, same terms as the row above, without DBnomics' ODbL. Approved by the maintainer 2026-09-13 as the replacement for every `dbnomics_eurostat` fetch and the source of the international pilot | daily |
| `ameco` | European Commission, DG ECFIN (AMECO) | `ameco` (to be built; endpoint confirmed when the adapter is written) | `economy-finance.ec.europa.eu` (AMECO) | **CC BY 4.0** under the Commission's reuse policy (Commission Decision of 12 December 2011), free of charge, unless an individual copyright notice says otherwise — credit required and changes indicated (the five series are rebased, so they carry the adapted-data notice). Supplied and approved by the maintainer 2026-09-13 as the replacement for `dbnomics_ameco` | daily |
| `dbnomics_ameco` | AMECO/EC (via DBnomics) — **being replaced by `ameco`** (maintainer, 2026-09-13) | `dbnomics` | `api.db.nomics.world/v22/series/AMECO` | TODO | daily |
