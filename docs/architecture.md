...
> **Update 2026-09-13, amended 2026-09-14.** This "As-is" section describes the repository before
> the rebuild and is kept as history. The daily run is now `daily_fetch.yml`: assemble the working
> database, every source sync, blocking validation, the exports, then offload (ADR 0006). A Dagster
> layer wraps those same scripts -- lineage, checks, freshness -- and since step 2 the workflow
> runs them through it, as one ephemeral Dagster execution inside the runner (`python -m
> orchestration.daily`). Since step 3 the offload is that execution's last asset, and the workflow
> then guards file sizes and opens the PR itself. GitHub Actions is still the only
