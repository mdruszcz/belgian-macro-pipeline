...
## Decision 1 — `z == 0` on a resolvable commune is loaded as `na`, value NULL, for every commune,
...
- `scripts/sync_police.py` gains `_dissolved_before()` and the `z == 0 -> na` branch inside `sync()`.
  Documented at length in the module's own docstring rather than only here, because a future editor
  reading that file needs the reasoning next to the code it governs.
- `scripts/correct_police_zero_placeholders.py` is new: the migration half of CLAUDE.md rule 1 (data
  ...
  the already-committed `data/police_observations.csv` in place. Tested to agree, row for row, with
  the fixed adapter run on equivalent synthetic raw JSON — so the next real re-sync from raw
  reproduces the corrected store exactly.
- Measured effect of running the correction on the real store: 348 rows had `z == 0` on a resolvable
  commune; 116 of those also belonged to a commune dissolved before its row's period and were
  dropped instead (dissolution takes precedence over the na rule), leaving 232 rows converted to
  `na`/NULL. 116 rows were dropped entirely for a dissolved commune (29 distinct communes: the 27
  dissolved 2025-01-01 plus Bastenaken-old and Bertogne, dissolved 2024-12-02, each carrying a 2025
  row for all four indicators). 7,205 of 7,553 rows were untouched. Per-indicator, `na` counts are
  `CAR_THEFT_PER_10K` 134, `THEFT_FROM_VEHICLE_PER_10K` 79, `HOUSE_BURGLARIES_PER_10K` 17,
  `DOMESTIC_VIOLENCE_PER_10K` 2 (fewer than the raw zero-count per indicator because some of each
  indicator's zero rows belonged to a now-dropped dissolved commune).
...
- Percentiles: an `na` cell drops out of its peer set the same way a `suppressed` cell already does,
  ...
  status generalisation above. Verified: Charleroi (Hainaut) has zero `CAR_THEFT_PER_10K` percentile
  rows after the fix; the national peer set for that indicator/period is 418, which is exactly the
  count of non-`na` `CAR_THEFT_PER_10K` 2025 rows in the corrected store (552 total rows, 134 `na`,
  418 remaining) — hand-computed, not merely asserted.
...
- The genuine-zero loss (Decision 1) has no mitigation inside this pipeline. If the federal police's
  written confirmation (in progress, docs/data_catalog.md) ever describes the source data at a level
  that distinguishes a placeholder from a true zero, this decision should be revisited — that is the
